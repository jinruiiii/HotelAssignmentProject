...
public class Rooms implements Serializable {
    ...
    Rooms(){
        setRooms(new Room[48]);
    }

    public void addRoom(int num, String roomNum, String roomType, Boolean wifi, Boolean smoking, Boolean view){
        switch(roomType){
            case "SingleRoom":
                getRooms()[num - 1] = new SingleRoom(roomNum, wifi, smoking, view);
                break;
            case "DoubleRoom":
                getRooms()[num - 1] = new DoubleRoom(roomNum, wifi, smoking, view);
                break;
                
            case "Deluxe":
                getRooms()[num - 1] = new Deluxe(roomNum, wifi, smoking, view);
                break;
            case "VIPSuite":
                getRooms()[num - 1] = new VIPSuite(roomNum, wifi, smoking, view);
                break;
    ...
    public void displayRooms(){
        ...
        for (int i = 0; i < 48; i++){
            System.out.printf("%d, %s, %b, %b, %b", getRooms()[i].getRoomNum(), getRooms()[i].getRoomType(), getRooms()[i].getWifi(), getRooms()[i].getSmoking(), getRooms()[i].getView());
            System.out.println();
        }
    }

    public Room getRoom(int roomNum){
        return getRooms()[roomNum - 1];
    }
    ...
    public ArrayList<String> printVacantRoomsByType(Room.RoomType rt){
        ...
        double total = 0;
        for (int i = 0; i < 48; i++){
            if (getRooms()[i].getRoomType() == rt){
                total += 1;
                if (getRooms()[i].getStatus() == Room.RoomStatus.valueOf("VACANT")){
    ...
    public ArrayList<String> printVacantRoomWithInfo(Room.RoomType rt){
        ...
        for (int i = 0; i < toReturn.size(); i++){
            room = getRooms()[deformatRoomNum(toReturn.get(i)) - 1];
